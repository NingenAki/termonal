package ningenaki.inc.termonal.client.singletons;
...
public class Words {
    ...
    private Words() {
        try {
            if (lists == null) {
                try (InputStream stream = Words.class.getClassLoader().getResourceAsStream("palavras5.json")) {
                    if (stream == null) {
                        throw new IllegalStateException("palavras5.json not found");
                    }
                    lists = mapper.readValue(stream, new TypeReference<List<List<String>>>() {
                    });
                }
